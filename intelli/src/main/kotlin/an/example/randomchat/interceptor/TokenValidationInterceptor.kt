package an.example.randomchat.interceptor
...
@Component
class TokenValidationInterceptor (private val userContextHolder: UserContextHolder) : HandlerInterceptor{
    ...
    private fun handleToken(
        ...
        val nickName = JWTUtil.extractNickName(jwt)
        val id = JWTUtil.extrackId(jwt)

        userContextHolder.set(nickName, id)

        true
    }catch (e: Exception){
        logger.error("토큰검증실패. token = $token", e)
        response.sendError(401)
        false
    }
    ...
    companion object{
        ...
        private val DEFAULT_ALLOWED_API_URLS = listOf("POST" to "/api/v1/randomchat/signin")
    }
